...
class ExamRoom extends BaseLogic
{
    /**
     * 获取用户列表
    ...
    public function getList(array $where, $order = [], $select = '*')
    {
        ...
        if (!empty($pageSize) && $pageSize > 0) {
            if ($total > 0) {
                ...
                $data = Db::table('x2_examroom')->limit($page * $pageSize, $pageSize)->where($where)->order($orderSort)->select();
            }
        } else {
            $data = Db::table('x2_examroom')->where($where)->order($orderSort)->select();
        }
        return array(
            'total' => $total,
            'list'  => $data,
