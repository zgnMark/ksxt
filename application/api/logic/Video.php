...
class Video extends BaseLogic
{
    ...
    public function getList(array $where, $order = [], $select = '*')
    {
        ...
        if (!empty($where['datemin'])) {
            $map['create_time'] = array('egt', $where['datemin'] . ' 00:00:00');
        }
        if (!empty($where['datemax'])) {
            $map['create_time'] = array('elt', $where['datemax'] . ' 23:59:59');
        }
        if (!empty($where['datemin']) && !empty($where['datemax'])) {
            $map['create_time'] = array('between', $where['datemin'] . ' 00:00:00' . ',' . $where['datemax'] . ' 23:59:59');
        }
        if (!empty($where['title'])) {
            $ids = Db::table('x2_video_resources')->where('title','like' ,$where['title'] . '%')->column('id');
            $map['video_id'] = array('in', $ids);
        }
        ...
        //获取总数
        $total = Db::table('x2_watch_log')->where($map)->count();
        ...
        if (!empty($pageSize) && $pageSize > 0) {
            if ($total > 0) {
                ...
                $data = Db::table('x2_watch_log')->limit($page * $pageSize, $pageSize)->where($map)->order($orderSort)->select();
            }
        } else {
            $data = Db::table('x2_watch_log')->where($map)->order($orderSort)->select();
        }
        return array(
            'total' => $total,
            'list'  => $data,
