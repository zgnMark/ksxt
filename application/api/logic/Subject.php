...
class Subject extends BaseLogic
{
    /**
     * 获取课程列表
    ...
    public function getList(array $where, $order = [], $select = '*')
    {
        ...
        //获取总数
        $total = Db::table('x2_subject')->where($where)->count();
        ...
        if (!empty($pageSize) && $pageSize > 0) {
            if ($total > 0) {
                ...
                $data = Db::table('x2_subject')->limit($page * $pageSize, $pageSize)->where($where)->order($orderSort)->select();
            }
        } else {
            $data = Db::table('x2_subject')->where($where)->order($orderSort)->select();
        }
        return array(
            'total' => $total,
            'list'  => $data,
